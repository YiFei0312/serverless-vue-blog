  ...
  <div class="bindex">
    <el-container>
      <el-header>
        <!-- 搜索框开始 -->
        <el-col :span="1">
          <el-input size="small" v-model="key" placeholder="请输入内容"></el-input>
        </el-col>
        <el-col :span="4">
          <el-button type="primary" icon="el-icon-search" @click="searchbytitle()" size="small">搜索</el-button>
        </el-col>
        <!-- 搜索框结束 -->
        <div style="size:80px">
          <el-dropdown @command="handleCommand">
          <span class="el-dropdown-link" style="color: #e56d79">
            <i class="el-icon-user-solid el-icon--right" id="iconx"></i>
            你好! {{ userName }}
          </span>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-if="userRole==1" command="myarticle">我的文章</el-dropdown-item>
            <el-dropdown-item v-if="userRole==1" command="addarticle">添加文章</el-dropdown-item>
      ...
      <el-main>
        ...
        <el-row :gutter="20">
                  ...
                  <el-table :data="articleList" style="width: 100%" class="table">
                    ...
                    <el-table-column prop="articleTitle" label="文章标题" width="230">
                      <template slot-scope="scope">
                        <div
                          @click="
                            toReadArticle(scope.row.accessNum, scope.row.id)
                          "
          ...
          <el-col :span="5">
            ...
            <el-card style="margin-top: 10px; height: 880px; width: 70% ">
              <div v-for="item in categoryList" 
                :key="item.id">
                <el-button 
                @click="search(item.id)"
                 icon="el-icon-edit"
                 :style="{background:listColor(item.id)}"
                 round="true"
                 class="buttonel"
                 >{{item.categoryName}}</el-button><br><br>
              </div>            
